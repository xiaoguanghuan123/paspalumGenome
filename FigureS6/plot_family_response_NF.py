...
dfout = pd.DataFrame()
fig,axes = plt.subplots(2, 5, figsize=(14,7), tight_layout =True)
a=0
for afamily in dfPvExpandNFDEG.index: 
    ...
    dfmelt = meltdataframeForboxplot(afamily,dfPvExpandNFDEG)
    ...
    if not len(dfmelt)==0:
        ...
        if afamily==7:
            ...
            ax.set_title(title , fontsize=10)
            print(afamily, title)
        dfmelt['annotation'] = title   
        dfout = dfout.append(dfmelt)
        
        #print(afamily,dfPvExpandGene.loc[afamily]['function'])
dfout.to_csv('NF_FamilyResponse.csv')
plt.savefig('PvSpecificExpandedFamilyNFDEG.svg')
plt.savefig('PvSpecificExpandedFamilyNFDEG.png',dpi=300)
