...
print(dfgene)
dfgene.to_csv('SLC_Gene_fc.csv')
plotOrder = ['FV_VS_F','NV_VS_N']
fig, axes = plt.subplots(1,3, figsize=(4,3.4), sharey=True, tight_layout = True) 
downgeneList = list(dffunc.index.unique()) #['SLC2C-8a','SLC23-2','SLC2C-8b', 'SLC35F1']

for i, agene in enumerate(downgeneList[1:]):
    ...
    ax.set_ylabel('')
    if i == 0:
        ax.set_ylabel('Log2 fold change (+ValA/-ValA)',size=10)  
    else: ax.set_ylabel('')   #$\it{text you want to show in italics}$
output =  'SLC_fc.svg'.format(geneName)
plt.savefig(output)


agene = downgeneList[0]
geneName = 'SLC2C-8A'
df = dfgene.loc[agene]
fig, ax = plt.subplots(figsize=(1.8,3.4), tight_layout = True) 
plt.setp(ax.spines.values(), linewidth=1.5)
plt.setp(ax.spines.values(), linewidth=1.5)
ax.spines['top'].set_visible(False)
ax.spines['right'].set_visible(False)
...
ax.set_ylabel('Log2 fold change (+ValA/-ValA)',size=10)
ax.set_ylim(0,1)
output =  'SLC2C-8A_fc.svg'.format(geneName)
plt.savefig(output)
