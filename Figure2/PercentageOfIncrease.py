...
sns.boxplot(x = "Species", y='P', hue="Treatment", data=dfspP, ax=axes[0], dodge=True, fliersize=0,linewidth=1, palette = 'Set2', order = spList)
sns.stripplot(x = "Species", y='P', hue="Treatment", data=dfspP, dodge=True, linewidth=1, s=1.5,order = spList, palette = 'Set2', ax=axes[0])

sns.boxplot(x = "Species", y='N', hue="Treatment", data=dfspN, ax=axes[1], dodge=True, fliersize=0,linewidth=1, palette = 'Set2', order = spList)
sns.stripplot(x = "Species", y='N', hue="Treatment", data=dfspN, dodge=True, linewidth=1, s=1.5,order = spList, palette = 'Set2', ax=axes[1])

axes[0].get_legend().remove()
h, l = axes[1].get_legend_handles_labels() #[2:]#.remove()
axes[1].legend(h[:2], l[:2])
axes[0].set_xlabel('')
axes[1].set_xlabel('')
...
axes[0].set_xticklabels(['Z. mays', 'P. vaginatum'], rotation = 35, ha = 'right', fontstyle = 'italic')
axes[1].set_xticklabels(['Z. mays', 'P. vaginatum'], rotation = 35, ha = 'right', fontstyle = 'italic')

plt.title('Full condition')
