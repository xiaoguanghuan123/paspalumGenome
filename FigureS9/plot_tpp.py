...
###############
#trppDict = ['trpp8']
geneGroup = 'trpp' #'trehalase' #'trpp6'
...
dfexp.to_csv('ZmTPP_TPM.csv')
dfsyn_melt = dfsyn_melt.join(dfexp, on='geneID')
dfsyn_melt.to_csv('ZmTPP_TPM_Melt.csv')
...
for agene in dfsyn_melt.index.unique():
    ...
    sns.boxplot(x='Species',y='TPM', hue='condition', hue_order=['F','N','P'] ,data=dfgene,fliersize=0, ax=ax)
    sns.stripplot(x='Species',y='TPM', hue='condition', hue_order=['F','N','P'] ,data=dfgene, jitter=True, dodge=True, linewidth=1,edgecolor='gray',ax=ax)
    ...
    labels=['Full','-N','-P']
    l = ax.legend(handles[:3], labels, loc=0, handlelength=1, borderpad=0.5)
    ax.set_xlabel('')
    ax.set_title(agene)
