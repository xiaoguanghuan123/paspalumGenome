...
yDict = {'Maize':6, 'Sorghum':6, 'Paspalum':18}
for i, species in enumerate(spList):
    ...
    sns.stripplot(x = "Condition", y = met, hue = 'Treatment', data=dfsp, dodge = True,  jitter = 0.05,  s=3,  palette = 'Set2', ax=ax)
    ylim = yDict[species]
    ax.set_ylim(0,ylim)
    ax.set_ylabel('Shoot to root ratio',size=10)    
    ax.set_xlabel(species)
    ax.get_legend().remove()
    ax.set_xlabel("")
    ax.set_title('$\it{0}$ 21 dap'.format(spDict[species])) 
    ax.set_xticklabels(['Full', '-N', '-P'],size=10, color='k')
    for condition, grp in dfsp.groupby('Condition'):
    	vmax = grp[met].max() + 0.5
    	vheight = 0.5
    	gap = 0.3
